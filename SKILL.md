...
description: >
  ...
  覆盖：确定范围并和用户对齐结构骨架 → 批量采集元数据 → 按模板分章生成并自检 → 用户确认本地 Markdown 后同步到飞书文档。
metadata:
  requires:
    bins: ["hac"]
---
...
4. **判定模块类型和主-明细关系，跟用户对齐**（决定整篇结构，不能跳过）：
   - 模块类型：基础资料/数据维护 → 业务流程 → 混合，三选一（判定标准见 writing-guide.md「文档整体结构」）。
   - 哪些是主单据表、哪些是它的明细子表、哪些是被引用的资料表；业务流程模块还要划出有哪几条业务闭环（如销售报价/订单/发货/收款）。
   - 这一步靠表名 + 关联关系能推个大概，但闭环怎么切章节模型容易猜错，**先给用户一版结构骨架让他确认/修正，再开始采集**。
5. **批量解析 table_id**：用户确认的是表名，后续采集要纯数字 `table_id`。先用 `hac table +resolve-id --table <表名>` 把范围内每张表的 id 解析出来列个对照表，避免逐条采集时反复卡在 ID 上（`collect_meta.py` 内部自己做 name→id，但走 hac 补查必须先有 id）。
6. 以上确认后才进入采集阶段。

这一步非常重要——不要自行判断哪些表属于这个模块、也不要自己拍板章节结构，让用户定。
...
另一个关键缺口：**脚本不采 form-layout、打印模板、自动编号规则、计算公式、字段显示条件**，而写作规范强依赖这些（字段表要按 form-layout 顺序排、要写"可打印XX""单号构成""这个值怎么算的"）。所以即便走脚本，这些也必须用上面的 hac `get-table --output-mode full` 和 `form-layout get` 单独补；脚本不是 hac 的等价替代，只是自动化清单这一段的唯一来源。
...
表多、采集 JSON 大时，先把采集结果落盘成文件，按阶段一确认的章节骨架**一个章节一个章节地写**，不要把整个工作区的数据一次性塞进上下文，避免漏写和上下文爆掉。

**生成后先自检，再交用户**（对照下面清单逐项过，发现问题当场补，不要把能机检的错留给用户）：
- 字段覆盖：每张表采集到的字段都进了对应字段表，没有漏字段（系统过程字段除外）。
- 字段顺序：字段表顺序对齐 form-layout 的 `main_layout`，不是 API 原始顺序。
- 章节完整：每个二级章节都有"注意事项"和"常见问题/典型场景"，不是只在文末放一个全局的。
- 无技术术语：正文不出现 trigger / validator / automation / 数据触发 / item_trigger / config / node / API 等黑话。
- 自动化落地：深查过的重点自动化，"点了之后发生什么"写进了对应章节，不是只列了个按钮名。
- 跨模块引导语带了"为什么"，常见问题小标题是真人口语提问（见 writing-guide 对应规则）。

自检通过后交用户审阅，标注 `[待确认]` 和 `[待补充]` 的地方需要用户校正。
...
3. **hac 认证失败/未安装时，脚本只能兜底表清单/字段结构/自动化清单这三样** — 脚本**不覆盖** form-layout、打印模板、自动编号、计算公式、字段显示条件、审批流程；这些缺了写作规范就满足不了。所以脚本不是 hac 的全量替代，hac 不可用时要么修认证、要么把缺的项标 `[待补充]` 让用户口述，不能假装采全了
