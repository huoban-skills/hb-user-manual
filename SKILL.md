...
2. **浏览器里能打开的都自己截，且一律走 `scripts/browser.py`**：不分伙伴云还是外部平台（阿里云、企业微信、飞书等控制台），走查和截图都由自己完成，不要让用户代截。唯一的例外是**输入账号密码**：登录动作在 browser.py 的窗口里交给用户，登完继续代劳。**不要去接管用户日常用的浏览器**——那类工具截的图落不到指定路径，也没有脱敏和标框能力，做不成手册产物。外部平台截图前先跟用户确认哪些信息要脱敏（账号名、UID、密钥、实例地址等）。姓名、手机号、企业名称、地址等普通敏感文字默认用模糊脱敏，保留字段和数据行形态；密钥、令牌、密码等秘密必须用不透明块不可逆遮挡。控制台类 SPA 的随机 class 让选择器失效时，改用 `scripts/annotate.py` 按坐标处理。
...
2. 启动走查浏览器：
   ...
   手册涉及外部平台控制台（阿里云、企业微信等）时同理：用 `start --url <控制台地址>` 打开，让用户在**这个窗口**里登录，之后的导航和截图自己做。用户日常浏览器里已有的登录态在这里用不上，也不要为了省一次登录去接管那个浏览器。
...
- **入口能不能找到**：菜单名和界面对不上、入口藏在多级菜单、两张图之间有跳跃的地方，都挂了直达链接。
- **截图无可识别的真实客户敏感数据**（走查时优先用演示数据入图）；普通敏感文字已充分模糊，密钥、令牌和密码已不可逆遮挡。
- 脱敏选区取“敏感字符外沿 + 极小余量”，不按整个输入框、单元格或表单区域框选。单行值只框字形高度，多行值按实际文字行拆成多个窄模糊带，相邻字段和相邻记录分别框选；空值不打码。字段名、列名、输入框边界、行结构、提示文案和按钮必须完整清晰，模糊区与这些系统文字之间要留出可见空隙。
- 脱敏后按原尺寸做两轮复检：先确认原值完全不可辨认，再确认所有系统字段文字零遮挡。发现漏字时只沿原值的字形方向补选区，优先向下或左右扩，不得为了省事向字段名方向扩大，也不得把多个小框合并成大块。
- 图片引用无死链：每个 `![](images/…)` 都能在 images/ 里找到，且都是相对路径。
- **每张图的红框对得上它所在的那一步**：步骤写「点创建层」，框就该在那个按钮上，不能框到别处。
- images/ 里没有未被引用的废图，`.orig.png` / `.grid.png` 中间产物已清理。
...
**annotate.py**：给截图模糊脱敏、不可逆遮挡、标红框和裁剪，位置按百分比给。控制台类 SPA 的 class 是随机哈希，`browser.py` 的 `--blur` / `--mask` / `--highlight` 选择器匹配不上时用它。普通敏感文字用 `--blur`，密钥、令牌、密码等秘密才用 `--fill`。坐标全部相对裁剪后的成图；没先 `--grid` 量过就处理会直接报错。重复传入 `--blur` 可实现“一个值一个小框”或“长文本一行一个窄框”，`--blur-radius` 只调模糊强度，不应用来弥补选区过大或错位。

```bash
python3 scripts/annotate.py <图> --crop 10,75 --grid              # 先量
python3 scripts/annotate.py <图> --crop 10,75 --box 14,36,73,44 \
  --blur 10,20,24,23 --blur 10,25,35,28 --blur-radius 7 --fill 85,0,100,7
```
...
| `shot --path P [--selector S] [--highlight S] [--blur S] [--mask S] [--full-page]` | 截图；`--blur` 模糊普通敏感文字，`--mask` 不可逆遮挡秘密，`--highlight` 标红框 |
