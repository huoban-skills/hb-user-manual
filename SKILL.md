...
```bash
python3 scripts/collect_meta.py --profile <profile> --tables "表1,表2,..." --detail --output <path>.json
```
...
2. **hac 不可用时（认证失败/未安装），退回 `scripts/collect_meta.py`** — 复用 zdf-shared 认证配置批量采集
3. **以上都不通，再考虑其他 huoban 类 skill**（huoban-table、huoban-automation、zdf-huoban-button 等）逐表手动采集
